...
def main():
    ...
    # Step 1: Always try to load contacts from the file
    ...
    # Step 2: Check if the user wants to add more people
    if prompt_for_additional_contacts():
        # Step 3: Open the text editor for the user to add contacts
        ...
        # Step 4: Load the contacts from the file again and add to ContactsManager
        new_contacts = load_contacts_from_file()
        if new_contacts:
            contacts_manager.add_multiple_contacts(new_contacts)
            print(f"{len(new_contacts)} new contacts loaded from {CONTACTS_FILE}.\n")

    # Step 5: Send greetings to all contacts
